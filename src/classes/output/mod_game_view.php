...
require_once($CFG->dirroot . "/mod/game/cryptex/play.php");
require_once($CFG->dirroot . "/mod/game/view.php");
...
foreach ($questions as $key => $q) {
    ...
    if (($onlyshow == false) and ($showsolution == false)) {
        if (($game->param8 == 0) || ($game->param8 > $q->tries)) {
            $question .= ' &nbsp;<input class="btn-primary" type="submit" value="'.
            get_string( 'answer').'" onclick="OnCheck( '.$q->id.",msg{$q->id});\" />";
        }
    }
    echo "<div class=\" recit-display-4\" >$question</div>";

    if ($showsolution) {
        echo " <span class=\" recit-display-4\" >&nbsp;&nbsp;&nbsp;$q->answertext<B></b></span>";
    }
    echo '<br>';
}
